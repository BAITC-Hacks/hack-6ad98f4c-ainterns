# Граф денег

Локальная панель для изучения направленных переводов и аналитических признаков. Роль и приоритет — это **признаки и гипотеза для проверки**, они не доказывают виновность.

## Установка и запуск

Из корня репозитория (Python 3.10+):

```bash
python3 -m venv .venv
source .venv/bin/activate       # Windows: .venv\Scripts\activate
python -m pip install -r requirements.txt
python -m src.pipeline
streamlit run src/app.py
```

`python -m src.pipeline` — команда полного пересчёта. Она читает три parquet-файла из `data/` и формирует CSV в `output/`. На текущем наборе обрабатываются 2 248 узлов и создаются 89 кластеров; полный расчёт занимает около 11 секунд на проверенном ноутбуке. Запускайте команду из корня репозитория. Выходные файлы: `nodes_roles.csv`, `clusters.csv`, `top_nodes.csv`.

## Правила ролей

Роли — это объяснимые признаки формы графа и гипотезы для проверки, а не выводы о нарушениях. Классификация применяется в указанном порядке:

| Роль | Правило |
| --- | --- |
| `consolidator` | Не seed, минимум 2 входящих ребра и входящая сумма не меньше 1,25× исходящей. |
| `distributor` | Минимум 2 исходящих ребра и исходящая сумма не меньше 1,25× входящей. |
| `coordinator` | Минимум 4 инцидентных ребра, есть и входящие, и исходящие. |
| `transit` | Есть оба направления, а отношение исходящей к входящей сумме находится в диапазоне 0,5–2,0. |
| `terminal` | Есть входящее и нет исходящего ребра, но узел не на глубине 4. |
| `peripheral` | Остальные случаи, включая изолированные узлы и узлы на глубине 4. |

Для seed входящие потоки неполны, поэтому seed не получает `consolidator` только на основании неполного баланса. Узел на глубине 4 не считается `terminal` только потому, что выгрузка обрезана. Переводы ниже 5 000 KZT отсутствуют в исходных данных.

## Панель

- Топ `output/top_nodes.csv` с рангом, gid, ролью, приоритетом и `why`, включая seed и длину кратчайшего направленного пути (по данным pipeline-коммита `cb7a530`).
- Поиск gid в `output/nodes_roles.csv` и карточка с ролью, `role_score`, `priority_score`, кластером и `evidence`.
- Направленная окрестность из `data/edges.parquet`, стрелки идут от `src` к `dst`, цвет узла соответствует роли, выбранный узел выделен. Показаны до 80 ближайших рёбер для читаемости.
- Основной список «Кого проверить первым» показывает 20 узлов с рангом, приоритетом и разложенным объяснением `why`; цвет обводки узла в графе соответствует кластеру.
- Таблицы входящих и исходящих связей с суммами переводов и количеством транзакций (если поля присутствуют).
- Просмотр участников кластера при наличии разметки кластера.

Ожидаемые колонки: `nodes_roles.csv`: `gid`, `role`, `role_score`, `cluster_id`, `priority_score`, `evidence`; `clusters.csv`: `cluster_id`, `n_nodes`, `n_seed`, `sum_kzt_internal`, `top_gids`, `hypothesis`; `top_nodes.csv`: `rank`, `gid`, `role`, `priority_score`, `why`; `edges.parquet`: `src`, `dst`, `sum_kzt`, `n_tx`. `top_gids` — запятый-разделённый список числовых gid в одной CSV-ячейке (например, `"1024,2048,4096"`); стандартный CSV writer берёт такую ячейку в кавычки. Интерфейс проверяет обязательные поля и сообщает, если файл отсутствует, пуст или имеет другую схему.

## Ограничения данных

Текущая реализация пайплайна читает `depth` и сохраняет ребро агрегированно по паре узлов. Входящие потоки неполны, а порог отбора транзакций 5 000 KZT может исключать меньшие переводы. Без эталонной разметки нельзя оценить точность ролей и приоритетов. Всегда формулируйте результат как аналитические признаки и гипотезу для проверки; изучайте первичные операции и контекст.

## Устойчивость сети — результаты проверки пайплайна

На 2 248 узлах после удаления top-5 приоритетных узлов сеть распадается на 389 слабосвязных компонент — на 354 больше исходного числа. У 534 узлов теряется путь от seed. Полный расчёт занимает около 11 секунд на проверенном ноутбуке. Это стресс-сценарий для анализа структуры, а не прогноз последствий реального вмешательства. Подробности seed и кратчайшего направленного пути для каждого узла из top-20 находятся в `output/top_nodes.csv`, поле `why`.

## Схема решения

```mermaid
flowchart LR
    I[data/*.parquet] --> P[src/pipeline.py]
    P --> O[output/nodes_roles.csv]
    P --> C[output/clusters.csv]
    P --> T[output/top_nodes.csv]
    O --> A[src/app.py]
    C --> A
    T --> A
    I --> A
```

Подробнее: [solution-schema.md](solution-schema.md).
